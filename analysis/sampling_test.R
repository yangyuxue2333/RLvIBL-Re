...
score_table_long <- read_csv(paste(dest_dir, "up", "score_table.csv", sep = "/")) %>% melt(
  id.vars = "n", 
...
png(filename = "./figure/score_up_n100.png",  bg = "white", 
...
stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.1, position = position_dodge(width = 0.8)) +
stat_summary(fun = mean, geom = "text", aes(label = round(..y.., 4)), vjust = -1, position = position_dodge(width = 0.8)) +
labs(title = "Testing Score with Up (Repeated N = 100 times)",
    x = "score type", y = "mean score (N = 100)") +
ylim(0, 1) +
